...
public class MainActivity extends Activity implements OnTouchListener, CvCameraViewListener2 {
    ...
    private static final String    TAG                 = "HandPose::MainActivity"; //一般常數宣告
    ...
    public void onCameraViewStarted(int width, int height) {
        ...
        */
        ...
        Toast.makeText(this, caption, Toast.LENGTH_SHORT).show();
        //toast =即時訊息,約3秒後消失
        //Toast makeText (Context context, CharSequence 要顯字的訊息文字, int 顯示持續時間(Toast.LENGTH_SHORT=2.5秒)).show()

        Camera.Parameters cParams = mOpenCvCameraView.getParameters();
        cParams.setFocusMode(Camera.Parameters.FOCUS_MODE_INFINITY);
        //設定對焦模式,  FOCUS_MODE_INFINITY=無限遠對焦
    ...
    public boolean onTouch(View v, MotionEvent event) {
        ...
        //touchedRegionRgba為mRgba的子mat(submat)

        Mat touchedRegionHsv = new Mat();
        Imgproc.cvtColor(touchedRegionRgba, touchedRegionHsv, Imgproc.COLOR_RGB2HSV_FULL);
        //Imgproc.cvtColor(來源mat, 目的mat, Color_Conversion_Code);

        //以下算出觸碰區域的平均顏色
        mBlobColorHsv = Core.sumElems(touchedRegionHsv);
        //Core.sumElems(tmp) : 計算矩陣差的元素總和
        //Scalar scalar = Core.sumElems(mat)
        //System.out.println(scalar.toString())
        //結果為[R值總和, G值總和, B值總和, 透明度]
        //圖片間有變化，矩陣值會變大，否則則為0
        ...
        return false; // don't n eed subsequent touch events
    }
